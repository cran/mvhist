...
#   slight modifications 11 September 2023
#
# A variety of different histograms are given:
...
#
# The histograms work in any dimension d >= 2, though the plots may be difficult to
...
sphere <- mvmesh::UnitSphere( n=d, k=k, method="dyadic", p=p, positive.only=positive.only )
S <- sphere$S
H <- HrepCones( S )
...
if (normalize.by.area) {
  for (i in 1:length(value)) {
    value[i] <- tally$counts[i]/SimplicialCubature::SimplexSurfaceArea( t(S[,,i]) )
  }
  ...
  if (plot.type == "index") {
    ...
    if (plot.type=="radial") {  
      if (d==2) {
        ...
        for (j in 1:ncones) {
          mvmesh::DrawSimplex2d( S[,,j], label=j, show.labels=FALSE, mvmesh.type=sphere$mvmesh.type, ... )
          mid <- colMeans( S[,,j] )
          tmp <- 1+value[j]/scale
        ...
        open3d()
        for (j in 1:ncones) {
          mvmesh::DrawSimplex3d( S[,,j], label=j, show.labels=FALSE, mvmesh.type=sphere$mvmesh.type, ... )
          mid <- colMeans( S[,,j] )
    ...
    if (plot.type=="orthogonal") { # must have d=3
      ...
      for (j in 1:ncones) {
        a <- S[,,j]
        mvmesh::DrawSimplex3d( a, show.labels=FALSE, mvmesh.type=sphere$mvmesh.type, ... )
        mid <- colMeans( a )
        tmp <- value[j]/scale  
...
for (i in 1:nbins) {
  satisfy <- mvmesh::SatisfyHrep( x, H[,,i] )
